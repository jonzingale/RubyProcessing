require 'matrix'
BASES = (0...5).map{|i|2**i}.freeze # 1,2,4,8
ALL_POINTS = (0...2**5).inject([],:<<).freeze
...
def setup
  background(20) ; frame_rate 20 ;	size(1400,1080)
	...
	@coords = edges.map{|a|a.map{|x|("%05d" % x.to_s(2)).split('').map(&:to_i)}}
	[:fill,:stroke].each{|f|send(f,*@rand_c)}
end

def draw
	clear ; cos,sin = %w(cos sin).map{|s| eval("Math.#{s} #{(@i+=0.001)*2*PI}")}
	rotation = Matrix.rows([[cos**2,1,cos*sin,-sin,0],[-cos*sin,1,cos**2,0,-sin]])

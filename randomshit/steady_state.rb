...
def walker_x(t,p=width/4,q=height/2) # not moving
  ...
  [kick,snare].map do |b|
		...
		p,q = (min_p[0] <= 10 ? max_p : max_p[0] >= 60 ? min_p : max_p)
 		...
 		stroke(100,100,100,3) ; strokeWeight(3)
 		low_diff = pairs.map{|p|diff(p)}.min
		text(low_diff,300,200);
 end
end
